...
// ---------------------------------------------------------------------------
// The thrift solver, v0: consolidation across the selected plan.
//
// Soviet home economics as an objective function — generic planners optimize
// variety; the Bureau optimizes stretch. One cabbage, several dinners.
...
export interface PlanItem {
  ...
  unit: string | null;
  /** Recipe names this item serves — length ≥ 2 means it works a double shift. */
  usedBy: string[];
}
...
export interface ConsolidatedPlan {
  ...
  sharedCount: number;
  /** Sum of every (item, recipe) pairing — the "uses" the cart covers. */
  totalUses: number;
...
export function consolidatePlan(recipes: CanonRecipe[]): ConsolidatedPlan {
  ...
  for (const recipe of recipes) {
    for (const ingredient of recipe.ingredients) {
      ...
      existing.usedBy.push(recipe.name);
...
export function planToLineItems(plan: ConsolidatedPlan): Array<{ name: string; quantity?: number; unit?: string }> {
